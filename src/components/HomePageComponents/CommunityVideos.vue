<template>
  <v-card flat class="transparent">
    <v-card-title>
      Videos from the community
    </v-card-title>
    <v-sheet
      class="mx-auto transparent"
      max-width="900px"
      v-if="communityVideos.length !== 0"
    >
      <v-slide-group>
        <v-slide-item v-for="stream in communityVideos" :key="stream.id">
          <Thumbnail
            type="large"
            :stream="stream"
            :id="stream.id"
            v-if="$vuetify.breakpoint.mdAndUp"
          />
          <Thumbnail type="mobile" :stream="stream" v-else />
        </v-slide-item>
      </v-slide-group>
    </v-sheet>
    <v-sheet
      class="d-flex flex-column justify-center align-center transparent my-10"
      max-width="100%"
      v-else
    >
      <v-card class="transparent title text--secondary" flat
        >There is no community videos at the moment...</v-card
      >
      <v-card class="transparent text-center text--secondary" flat
        >Check again later!
      </v-card>
    </v-sheet>
  </v-card>
</template>

<script>
import Thumbnail from "../Reusables/Thumbnail";

export default {
  data: () => {
    return {
      communityVideos: [],
    };
  },
  components: {
    Thumbnail,
  },
  props: {
    videos: Object,
  },
};
</script>

<style></style>
